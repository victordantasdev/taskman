    ...
    <div className="flex justify-between items-center w-[100%]">
      <div className="flex justify-center items-center">
        <input
          ...
          onChange={() => {
            if (taskIsDone) {
              const newDoneTasks = doneTasks?.filter((t) => t !== task.id);
              localStorage.setItem('done-tasks', JSON.stringify(newDoneTasks));
              setDoneTasks(newDoneTasks);
            } else {
              const newDoneTasks = [...doneTasks ? doneTasks : [], task.id];
              localStorage.setItem('done-tasks', JSON.stringify(newDoneTasks));
              setDoneTasks(newDoneTasks)
            }
          }}
